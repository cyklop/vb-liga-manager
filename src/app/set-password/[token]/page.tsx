...
export default function SetInitialPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!token) {
      setError('Ungültiger oder fehlender Setup-Token.')
      return
    }
    ...
    try {
      ...
      if (response.ok) {
        setSuccess(data.message || 'Dein Passwort wurde erfolgreich festgelegt. Du kannst dich jetzt anmelden.')
        // Leite den Benutzer nach kurzer Verzögerung zur Login-Seite weiter
        setTimeout(() => {
          router.push('/login')
        }, 3000) // 3 Sekunden Verzögerung
      } else {
        setError(data.message || 'Fehler beim Festlegen des Passworts.')
      }
    } catch (error) {
          ...
          {!success ? (
            <>
              ...
              <Button className="mt-6" fullWidth type="submit" placeholder={undefined} disabled={isLoading} onPointerEnterCapture={undefined} onPointerLeaveCapture={undefined}>
                {isLoading ? 'Wird verarbeitet...' : 'Passwort festlegen & Anmelden'}
              </Button>
            </>
          ) : (
