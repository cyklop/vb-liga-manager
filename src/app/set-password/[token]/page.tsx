...
// Diese Seite ist fast identisch mit ResetPasswordPage,
// nur der Titel und der API-Endpunkt sind anders.
export default function SetInitialPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!token) {
      toast.error('Ungültiger oder fehlender Setup-Token.')
      return
    }
    ...
    try {
      // Rufe den neuen API-Endpunkt auf
      ...
      if (response.ok) {
        toast.success(data.message || 'Dein Passwort wurde erfolgreich festgelegt. Du wirst weitergeleitet...')
        // Leite den Benutzer nach kurzer Verzögerung zur Login-Seite weiter
        setTimeout(() => {
          router.push('/login')
        }, 3000) // 3 Sekunden Verzögerung
      } else {
        // Use toast for API errors
        toast.error(data.message || 'Fehler beim Festlegen des Passworts.')
      }
    } catch (error) {
    ...
    <div className="flex min-h-screen items-center justify-center bg-base-200 py-12 px-4 sm:px-6 lg:px-8">
      ...
      <div className="card w-full max-w-md bg-base-100 shadow-xl">
        <div className="card-body">
          ...
          <h2 className="card-title text-2xl justify-center">Passwort festlegen</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              ...
              disabled={isLoading}
            >
              {isLoading ? <span className="loading loading-spinner"></span> : 'Passwort festlegen & Anmelden'}
            </button>
          </form>
        </div>
      </div>
